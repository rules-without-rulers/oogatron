...
export async function syncIssueComments(
  ...
  if (s.phase === "backfill") {
    while (ctx.budget.canAfford(3)) {
      ...
      if (pageInfo.hasNextPage) {
        ...
        await persistPage(ctx, repo.name, "issue_comments", events, s);
      } else {
        s = {
          phase: "incremental",
  ...
  for (;;) {
    ...
    await persistPage(ctx, repo.name, "issue_comments", events, {
      phase: "incremental",
  ...
  while (ctx.budget.canAfford(3)) {
    ...
    try {
      ...
      if (
        ...
        ctx.budget.spend();
        await ctx.db.batch([
          syncStateUpsert(ctx.db, repo.name, "commit_comments", {
            cursor: null,
          }),
        ]);
        return false;
    ...
    await persistPage(ctx, repo.name, "commit_comments", events, {
      cursor: newCursor,
    });
